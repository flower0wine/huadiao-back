        ...
        try {
            ...
            for (FileItem fileItem : fileItems) {   //遍历，找到表单中每一个文件对应的<input>上传的文件数据
                if (fileItem.isFormField()) {   //这个<input>中的数据不是文件
                    ...
                    switch (name) {
                        case "getHistory" : {
                            HistoryInfer historyInfer = mysqlProcess.selectHistory(myUid);
                            System.out.println(historyInfer);
                            response.getWriter().write(JSON.toJSONString(historyInfer));
                            return;
                        }
                        case "otherUid" : {
                            otherUid = value;
                        ...
                        case "deleteNoteHistory" : {
                            mysqlProcess.deleteNoteHistory(otherUid, myUid, noteId);
                            return;
                        }
                        case "deleteFanjuHistory" : {
                            mysqlProcess.deleteFanjuHistory(otherUid, myUid);
                            return;
                        }
                    }
                }
