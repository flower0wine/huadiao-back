	...
	$.fn.fullpage = function (options) {
		...
		*/
		...
		$.fn.fullpage.moveSectionUp = function () {
			...
			// 当滚去页面小于一个页面的高度时，隐藏滚动条，否则显示
			if (prev.length && prev.height() >= window.pageYOffset ) {		// 这里做了修改，当滚动到第三张页面时，向上滚动将做限制
				scrollPage(prev, null, true);	// 向上滚动

				// 这里实现了一个效果，当滚动到第三张页面时显示滚送条，否则不显示
			...
			// 发现一处与想象中不符合·，当上面没有页面时，向上滚动滚动条会出现，
			// 原因是 向上滚动 prev 会为 undefined，上面的判断会出现空指针异常，这里进行类型判断，解决了问题
			if(typeof prev.height() == 'undefined'){	
				$('body').addClass('scrollbar');
			}

		};
			
		// 打开网页隐藏滚动条（通过更改 class 来完成）
		$('body').addClass('scrollbar');
		console.log("修改地方在 204 行左右");
		...
		$.fn.fullpage.moveSectionDown = function () {
			...
			if (next.length && next.height() >= window.pageYOffset) {	// 这里做了修改，当滚动到第三张页面时，向上滚动将做限制
